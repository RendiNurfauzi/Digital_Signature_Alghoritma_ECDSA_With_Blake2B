...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (signPdfVisible) {
        signPdfVisible.addEventListener('click', async function () {
            ...
            // Ensure the PDF has a form. Create one if it does not exist.
            const form = pdfDoc.getForm() || pdfDoc.createForm();

            // Check if a signature field already exists, use it or create a new one
            let signatureField;
            try {
            ...
            console.log('Tanda tangan ditambahkan ke anotasi PDF.');

            const pdfBytes = await pdfDoc.save();
            download(pdfBytes, "signed_document.pdf", "application/pdf");
            console.log('PDF ditandatangani dan diunduh.');
            showPopup("PDF ditandatangani Secara Terlihat dan diunduh.");
            downloadPublicKey(keyPair.getPublic('hex'));  // Unduh kunci publik
        });
    }
    ...
    if (signPdfInvisible) {
        signPdfInvisible.addEventListener('click', async function () {
            ...
            download(pdfBytes, "signed_document.pdf", "application/pdf");
            console.log('PDF ditandatangani dan diunduh.');
            showPopup("PDF ditandatangani Secara Tidak Terlihat dan diunduh.");
            downloadPublicKey(keyPair.getPublic('hex'));  // Unduh kunci publik
        });
    }
    ...
    if (verifyPdfButton) {
        verifyPdfButton.addEventListener('click', async function () {
            ...
            try {
                signatureField = form.getTextField('signatureField');
                const signature = signatureField.getText();
                if (!signature) {
                    console.error('Tidak ada tanda tangan ditemukan di anotasi PDF.');
                    showPopup("Tidak ada tanda tangan ditemukan di anotasi PDF.");
                    return;
                }
    
                console.log('Tanda tangan dari anotasi PDF:', signature);
                showPopup("Tanda tangan dari anotasi PDF:", signature);
                ...
                reader.onload = async function (e) {
                    ...
                    if (isValid) {
                        ...
                        showPopup("Verifikasi tanda tangan berhasil, tanda tangan valid.");
                    } else {
